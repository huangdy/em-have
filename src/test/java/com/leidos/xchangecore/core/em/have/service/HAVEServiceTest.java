 ...
 * 
 */
package com.leidos.xchangecore.core.em.have.service;
...
import com.leidos.xchangecore.core.em.have.service.impl.HAVEServiceImpl;
import com.leidos.xchangecore.core.em.util.BroadcastUtil;
import com.leidos.xchangecore.core.em.util.DigestTester;
...
public class HAVEServiceTest extends TestCase {
	...
	public void testSubmitHAVE() {
		...
		try {
			EdxlDeResponseDocument response = sendAndCheckHAVE(null);
	...
	public void testSubmitHAVEWithNoAgreement() {
		...
		try {
			EdxlDeResponseDocument response = sendAndCheckHAVEAndExpectExceptionOnSend(null);
	...
	public void expectXQuery() {
		try {
			...
			list.add(wp);
			EasyMock.expect(workProductService.getProductByTypeAndXQuery(EasyMock.eq(HAVEService.HAVE_PRODUCT_TYPE),
	...
	public void testSubmitHAVEFromFile() {
		...
		try {
			...
			EdxlDeResponseDocument response = sendAndCheckHAVE(fileName);
			...
			assertTrue("missing simple type for bed capacity", DigestTester.checkSimpleProperty(
					response.getEdxlDeResponse().getDigest(), DigestTester.ORGANIZATION_THING_TYPE, HOSPITAL_0_ID,
	...
	private EdxlDeResponseDocument sendAndCheckHAVE(String fileName) throws XmlException, IOException {
		...
		EasyMock.expect(workProductService.publishProduct(EasyMock.isA(WorkProduct.class))).andReturn(status);
	...
	private EdxlDeResponseDocument sendAndCheckHAVEAndExpectExceptionOnSend(String fileName)
	...
	public void addHospital(String name, Calendar now, HospitalStatusDocument have) {
		...
		hospital.addNewOrganization().addNewOrganizationInformation().addNewOrganisationName().addNewNameElement()
				.setStringValue(name);
		hospital.getOrganization().addNewOrganizationGeoLocation().addNewPoint().addNewPos()
				.setStringValue(DEFAULT_POINT_POS);
	...
	private void checkCommonHAVEResponseElements(EdxlDeResponseDocument response, Set<String> organizationNames) {
		...
		assertTrue("no Hospital 0 locatedAt",
				DigestTester.hasLocatedAtElement(digest, HOSPITAL_0_ID, HOSPITAL_0_GEO_LOCATION_ID));
		assertTrue("no Hospital 1 locatedAt",
				DigestTester.hasLocatedAtElement(digest, HOSPITAL_1_ID, HOSPITAL_1_GEO_LOCATION_ID));
		...
		assertTrue(DigestTester.checkOrganizationWhatElement(digest, HOSPITAL_0_ID, UCORE_CODESPACE_NS, "Organization",
				null, null));
		assertTrue(DigestTester.checkOrganizationWhatElement(digest, HOSPITAL_1_ID, UCORE_CODESPACE_NS, "Organization",
				null, null));
